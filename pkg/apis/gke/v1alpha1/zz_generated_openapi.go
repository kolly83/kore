		...
		Schema: spec.Schema{
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"version": {
						SchemaProps: spec.SchemaProps{
							Description: "Version is the initial kubernetes version which the cluster should be configured with. '-' gives the current GKE default version, 'latest' gives most recent, 1.15 would be latest 1.15.x release, 1.15.1 would be the latest 1.15.1 release, and 1.15.1-gke.1 would be the exact specified version. Must be blank if following release channel.",
							Type:        []string{"string"},
							Format:      "",
						},
					},
					"releaseChannel": {
						SchemaProps: spec.SchemaProps{
							Description: "ReleaseChannel is the GKE release channel to follow, must be UNSPECIFIED (uses default auto-upgrade as defined by GCP), STABLE (only battle-tested releases every few months), REGULAR (stable releases every few weeks) or RAPID (bleeding edge, not suitable for production workloads). If anything other than UNSPECIFIED, Version must be blank.",
							Type:        []string{"string"},
							Format:      "",
						},
					},
