...
type GKESpec struct {
	...
	// ReleaseChannel is the GKE release channel to follow, must be UNSPECIFIED (uses default auto-upgrade
	// as defined by GCP), STABLE (only battle-tested releases every few months), REGULAR (stable releases
	// every few weeks) or RAPID (bleeding edge, not suitable for production workloads). If anything other
	// than UNSPECIFIED, Version must be blank.
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:Required
