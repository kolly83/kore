...
// UpdateNodePools is called to add, remove and update node pools in the cluster
func (g *gkeClient) UpdateNodePools(ctx context.Context) (bool, error) {
	logger := log.WithFields(log.Fields{
		"name":      g.cluster.Name,
		"namespace": g.cluster.Namespace,
	})
	...
	for _, nodePoolSpec := range g.cluster.Spec.NodePools {
		...
		if !nodePoolExists {
			// Node pool added to spec, request addition to GKE
	...
	for _, nodePool := range state.NodePools {
		nodePoolSpec, nodePoolInSpec := func() (*gke.GKENodePool, bool) {
		...
		if !nodePoolInSpec {
			...
			if err != nil {
				logger.WithError(err).WithField("cluster", g.cluster.Name).WithField("nodePool", nodePool.Name).Error("trying to delete node pool")
		...
		if g.cluster.Spec.ReleaseChannel == "UNSPECIFIED" {
			...
			if updatePool {
				nodePoolLogger.WithFields(log.Fields{
					"nodePoolVersion": nodePool.Version,
					"specVersion":     nodePoolSpec.Version,
				}).Info("Node pool upgrade required to match spec")
			}

			// For node pools, we need to check additionally if there's a node pool upgrade available to
			// keep it in sync with master, if auto-upgrade is enabled for the node pool and the version
			// has been set to '-'.
			if !updatePool && nodePoolSpec.EnableAutoupgrade && nodePoolSpec.Version == "-" && nodePool.Version != state.CurrentMasterVersion {
				updatePool = true
				nodePoolLogger.WithFields(log.Fields{
					"nodePoolVersion": nodePool.Version,
					"masterVersion":   state.CurrentMasterVersion,
				}).Info("Node pool upgrade required to match master")
			}
		}
		...
		if !updatePool && nodePoolSpec.ImageType != nodePool.Config.ImageType {
			updatePool = true
			nodePoolLogger.WithFields(log.Fields{
				"nodePoolImageType": nodePool.Config.ImageType,
				"specImageType":     nodePoolSpec.ImageType,
			}).Info("Node pool image type update required")
		}

		if updatePool {
			// Request upgrade / image type change
...
func (g *gkeClient) CreateUpdateDefinition(state *container.Cluster) (*container.UpdateClusterRequest, error) {
	...
	// Update release channel if changed. Note the release channel returned from the GCP API is empty when
	// the actual release channel is set to UNSPECIFIED, so need to specifically check for that:
	if state.ReleaseChannel.Channel != g.cluster.Spec.ReleaseChannel && !(state.ReleaseChannel.Channel == "" && g.cluster.Spec.ReleaseChannel == "UNSPECIFIED") {
...
func (g *gkeClient) CreateDefinition() (*container.CreateClusterRequest, error) {
	...
	resource := &container.Cluster{
		...
		ReleaseChannel: &container.ReleaseChannel{
			Channel: cluster.Spec.ReleaseChannel,
		},
	...
	return &container.NodePool{
		...
		Management: &container.NodeManagement{
			...
			AutoUpgrade: func() bool {
				...
				if cluster.Spec.ReleaseChannel != "UNSPECIFIED" {
					return true
				}
