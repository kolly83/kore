...
class ClusterPage extends React.Component {
  ...
  fetchComponentData = async () => {
    ...
    let [ namespaceClaims, services, serviceCredentials, serviceKinds ] = await Promise.all([
      api.ListNamespaces(team),
      this.servicesEnabled() ? api.ListServices(team) : Promise.resolve({ items: false }),
    ...
    const applicationServices = services.filter(s => s.spec.cluster.namespace === this.props.cluster.metadata.namespace && s.spec.cluster.name === this.props.cluster.metadata.name) // TODO: filter by app services
    ...
    return { namespaceClaims, services, applicationServices, serviceCredentials, serviceKinds, revealBindings }
  }
  ...
  handleApplicationServiceCreated = (applicationService) => {
    this.setState((state) => {
      return {
        createApplicationService: false,
        ...
        {this.servicesEnabled() && (
          <>
            <Card
              ...
              extra={<Button type="primary" disabled={!hasActiveNamespaces} onClick={() => this.setState({ createApplicationService: true })}>+ New application service</Button>}
            >
