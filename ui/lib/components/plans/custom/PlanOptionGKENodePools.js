...
export default class PlanOptionGKENodePools extends PlanOptionBase {
  ...
  addNodePool = () => {
    ...
    if (this.props.plan.releaseChannel && this.props.plan.releaseChannel !== 'UNSPECIFIED') {
      newNodePool.version = ''
    ...
    // Open the draw to immediately edit the new node pool:
    this.setState({
  ...
  render() {
    ...
    let ngNameClash = false, versionFollowMaster = false
    if (selectedNodePool) {
    ...
    if (plan.releaseChannel && plan.releaseChannel !== 'UNSPECIFIED') {
      followingReleaseChannel = true
    }

    const allMachineTypes = selectedNodePool ? getSupportedMachineTypes() : null
    const selectedMachineTypeFamilyInfo = selectedNodePool ? supportedMachineTypes[allMachineTypes.index[selectedNodePool.machineType][0]].about : null
              ...
              <Collapse defaultActiveKey={['basics','compute','metadata']}>
                <Collapse.Panel key="basics" header="Basic Configuration (name, versions, sizing)">
                  <Form.Item label="Name" help="Unique name for this group within the cluster">
                    <Input value={selectedNodePool.name} onChange={(e) => this.setNodePoolProperty(selectedNodePoolIndex, 'name', e.target.value)} readOnly={!editable} />
                  ...
                  {followingReleaseChannel ? (
                    ...
                    <> 
                      ...
                      <Form.Item label="Version" help="Set the Kubernetes version for this node pool">
                        <Checkbox checked={versionFollowMaster} onChange={(e) => this.setNodePoolProperty(selectedNodePoolIndex, 'version', e.target.checked ? '-' : ' ')} disabled={!editable} /> Same as master (recommended)
